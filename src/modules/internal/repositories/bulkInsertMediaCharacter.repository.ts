...
import { AnimeCharacter } from "../../../config/apis/myanimelist/types/animeCharacters.type";

export const bulkInsertMediaCharacterRepository = async (mediaId: string, characters: AnimeCharacter[]) => {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // Build lookup map
      const characterMap = new Map(insertedChar.map((c) => [c.mal_id!, c.id]));
      const staffMap = new Map(insertedStaff.map((s) => [s.mal_id!, s.id]));

      // Connect media with characters
      const mediaCharacters = characters.map((c) => {
        const characterId = characterMap.get(c.character.mal_id);
        if (!characterId) throw new AppError(500, `Character ${c.character.mal_id} not found`);
